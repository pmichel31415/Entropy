...
void Store_tree::open(char* filename){
	...
	if (f == 0){
		...
		hits = new Simple_tree(0, 0);
		hits->read_from_file(f);
		fclose(f);
	}
...
bool Store_tree::exists(queue<char> seq){
	return hits->is_in(seq);
}

void Store_tree::create(queue<char> seq, llu nb){
	hits->add(seq, nb);
}

// Access or modify hits
llu Store_tree::get_hits(queue<char> seq){
	return hits->get(seq);
}
void Store_tree::add_hits(queue<char> seq, llu nb){
	hits->add(seq, nb);

}

// Access proba
double Store_tree::get_proba(queue<char> seq){
	return hits->get_p(seq);
}

char Store_tree::next(queue<char> seq){
